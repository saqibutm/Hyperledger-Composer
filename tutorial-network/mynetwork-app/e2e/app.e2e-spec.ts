...
describe('Starting tests for mynetwork-app', function() {
  ...
  it('website title should be mynetwork-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('mynetwork-app');
    })
  });
  ...
  it('navbar-brand should be mynetwork-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('mynetwork-app');
    });
  });
