...
describe('Starting tests for tutorial-network', function() {
  ...
  it('website title should be tutorial-network', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('tutorial-network');
    })
  });
  ...
  it('navbar-brand should be tutorial-network',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('tutorial-network');
    });
  });
