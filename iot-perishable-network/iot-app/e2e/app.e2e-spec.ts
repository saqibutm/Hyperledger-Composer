...
describe('Starting tests for iot-app', function() {
  ...
  it('website title should be iot-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('iot-app');
    })
  });

  it('network-name should be iot-perishable-network@0.1.12',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('iot-perishable-network@0.1.12.bna');
    });
  });

  it('navbar-brand should be iot-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('iot-app');
    });
  });
